...
본 보고서는 동시성(Concurrency) 환경에서 게시글 조회수 증가 로직을 수행할 때 발생하는 **데이터 불일치(Data Inconsistency)** 현상을 관측하고, 다양한 동시성 제어 기법을 적용하여 정합성과 성능 간의 트레이드오프(Trade-off)를 분석하는 것을 목적으로 한다.
